...
void lightKeys(uint8_t keycode[4], uint8_t score)
{
    ...
    for (int i = 0; i < 4; i++) {
        switch (keycode[i]) {
            case 0x07: // 'd'
                for (int x = 24 - click_width; x < 24 + click_width; x++) {
                    for (int y = ROWS - 33; y < ROWS - 29; y++) {
                        drawPixel(x, y, color);
                    }
                }
                break;
            case 0x09: // 'f'
                for (int x = 74 - click_width; x < 74 + click_width; x++) {
                    for (int y = ROWS - 33; y < ROWS - 29; y++) {
                        drawPixel(x, y, color);
                    }
                }
                break;
            case 0x0D: // 'j'
                for (int x = 124 - click_width; x < 124 + click_width; x++) {
                    for (int y = ROWS - 33; y < ROWS - 29; y++) {
                        drawPixel(x, y, color);
                    }
                }
                break;
            case 0x0E: // 'k'
                for (int x = 174 - click_width; x < 174 + click_width; x++) {
                    for (int y = ROWS - 33; y < ROWS - 29; y++) {
                        drawPixel(x, y, color);
                    }
                }
